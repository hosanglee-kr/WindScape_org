...
bool CL_C10_ConfigManager::loadMotionConfig(ST_A10_MotionConfig& p_cfg) {
    ...
    */
    ...
    p_cfg.ble.rssi.persist_count =
        j["ble"]["rssi"]["persist_count"] | 5;
    p_cfg.ble.rssi.exit_delay_sec =
        j["ble"]["rssi"]["exit_delay_sec"] | 12;
...
bool CL_C10_ConfigManager::patchSystemFromJson(
    ...
    if (!j_sys.isNull()) {
        ...
        if (!j_log.isNull()) {
            ...
            if (strlen(v_lv) > 0 &&
                strcmp(v_lv, p_config.system.logging.level) != 0) {
                strlcpy(p_config.system.logging.level,
                        v_lv,
                        sizeof(p_config.system.logging.level));
                v_changed = true;
            }
        }
    }
    ...
    if (!j_sec_root.isNull()) {
        ...
        if (strlen(v_key) > 0 &&
            strcmp(v_key, p_config.security.api_key) != 0) {
            strlcpy(p_config.security.api_key,
                    v_key,
    ...
    if (v_changed) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_INFO,
                           "[C10] System config patched (Memory Only). Dirty=true");
    }
...
bool CL_C10_ConfigManager::patchWifiFromJson(
    ...
    if (!j_ap.isNull()) {
        ...
        if (strlen(v_ssid) > 0 &&
            strcmp(v_ssid, p_config.ap.ssid) != 0) {
            strlcpy(p_config.ap.ssid,
                    v_ssid,
                    sizeof(p_config.ap.ssid));
            v_changed = true;
        }

        const char* v_pwd = j_ap["password"] | "";
        if (strlen(v_pwd) > 0 &&
            strcmp(v_pwd, p_config.ap.password) != 0) {
            strlcpy(p_config.ap.password,
                    v_pwd,
                    sizeof(p_config.ap.password));
            v_changed = true;
        }
    }
    ...
    if (!j_sta.isNull()) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_DEBUG,
                           "[C10] WiFi STA array fully replaced.");
    }

    if (v_changed) {
        _dirty_wifi = true;
        CL_D10_Logger::log(EN_L10_LOG_INFO,
                           "[C10] WiFi config patched (Memory Only). Dirty=true");
    }
...
bool CL_C10_ConfigManager::patchMotionFromJson(
    ...
    if (!j_ble.isNull()) {
        ...
        JsonArrayConst j_devices =
            j_ble["trusted_devices"].as<JsonArrayConst>();
        if (!j_devices.isNull()) {
            ...
            v_changed = true;
            CL_D10_Logger::log(EN_L10_LOG_DEBUG,
                               "[C10] Motion Trusted Devices array fully replaced.");
        }
    }
    ...
    if (v_changed) {
        _dirty_motion = true;
        CL_D10_Logger::log(EN_L10_LOG_INFO,
                           "[C10] motion config patched (Memory Only). Dirty=true");
    }
