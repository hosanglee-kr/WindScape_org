/*
 * ------------------------------------------------------
 * 소스명 : CT10_Control_control_026.cpp
 * 모듈약어 : CT10
...
void CL_CT10_ControlManager::startOverridePreset(const char* p_presetCode,
    ...
    if (!v_ok || !v_resolved.valid) {
        CL_D10_Logger::log(EN_L10_LOG_WARN, "[CT10] startOverridePreset resolve failed (%s,%s)",
...
void CL_CT10_ControlManager::applyManualResolved(const ST_A10_ResolvedWind_t& p_wind, uint32_t p_seconds) {
    ...
    CL_D10_Logger::log(EN_L10_LOG_INFO, "[CT10] applyManual: preset=%s style=%s (sec=%lu)",
                       p_wind.presetCode, p_wind.styleCode, (unsigned long)p_seconds);
}
...
bool CL_CT10_ControlManager::tickUserProfile() {
    ...
    return tickSegmentSequence(v_profile.repeatSegments,
...
bool CL_CT10_ControlManager::tickSchedule() {
    ...
    return tickSegmentSequence(v_schedule.repeatSegments,
                               v_schedule.repeatCount,
                               v_schedule.segments,
                               v_schedule.seg_count,
                               scheduleSegRt);
}
...
void CL_CT10_ControlManager::applySegmentOn(const ST_A10_ScheduleSegment_t& p_seg) {
    ...
    if (p_seg.mode == EN_A10_SEG_MODE_FIXED) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_INFO, "[CT10] SegmentOn(SCH) FIXED duty=%.1f%%",
                           p_seg.fixed_speed);
        return;
    }
    ...
    if (v_ok && v_resolved.valid) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_INFO,
                           ...
                           (unsigned)p_seg.on_minutes,
                           (unsigned)p_seg.off_minutes);
    } else {
...
void CL_CT10_ControlManager::applySegmentOn(const ST_A10_UserProfileSegment_t& p_seg) {
    ...
    if (p_seg.mode == EN_A10_SEG_MODE_FIXED) {
        ...
        CL_D10_Logger::log(EN_L10_LOG_INFO, "[CT10] SegmentOn(PROFILE) FIXED duty=%.1f%%",
                           p_seg.fixed_speed);
        return;
    }
