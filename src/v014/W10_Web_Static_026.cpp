 ...
 * - [리팩토링]: 페이지 목록을 구조체 배열로 통합하고 라우팅 로직 간소화
 * - [추가]: SC10_common_001.css 및 공통 JS 경로를 자동으로 포함하도록 보완
 * ------------------------------------------------------
 * [구현 규칙]
...
// 페이지 파일 경로 정의 구조체
typedef struct {
	const char* uri;   // 단축 URI (예: "/dashboard", "/config")
	const char* html;  // HTML 파일 경로
	const char* css;   // 전용 CSS 파일 경로 (NULL 허용)
	const char* js;    // 전용 JS 파일 경로 (NULL 허용)
} ST_W10_Page_t;
...
// 정적 페이지 목록: 모든 페이지 정보를 구조체 배열로 통합 관리
static const ST_W10_Page_t s_pages_static[] = {
	// uri       , html                        , css                        , js
	{"/",         "/html/SC10_main_019.html",   "/html/SC10_main_019.css",   "/html/SC10_main_019.js"  }, // MAIN (설정 오버라이드 가능하도록 아래 라우팅 로직에서 별도 처리)
	{"/dashboard","/html/SC10_dashboard_001.html", NULL,                       NULL                      }, // 대시보드
	{"/chart",    "/html/SC10_chart_006.html",  "/html/SC10_chart_005.css",  "/html/SC10_chart_005.js" }, // 차트 모니터링
	{"/sim_details","/html/SC10_sim_details_001.html", NULL,                   NULL                      }, // 시뮬 설정
	{"/profiles", "/html/SC10_profile_001.html", NULL,                       NULL                      }, // 프로파일 관리
	{"/schedules","/html/SC10_schedule_001.html", NULL,                       NULL                      }, // 스케줄 관리
	{"/user_profiles","/html/SC10_user_001.html", NULL,                       NULL                      }, // 사용자 프로필
	{"/settings", "/html/SC10_settings_001.html", NULL,                       NULL                      }, // 시스템 설정
	{"/config",   "/html/SC10_config_025.html",   "/html/SC10_config_025.css", "/html/SC10_config_025.js"}, // Config 설정
	{"/diag",     "/html/SC10_diag_025.html",     "/html/SC10_diag_025.css",   "/html/SC10_diag_025.js"  }  // 시스템 진단
};
// 페이지 목록 크기 계산
...
// ------------------------------------------------------
// Static routing table (개별 파일 라우팅 정보를 저장)
// ------------------------------------------------------
...
// 정적 라우팅 테이블 배열 (최대 라우트 수 증가: 페이지 수 * 5 (HTML/CSS/JS/공통CSS/공통JS) + 1 (루트 리다이렉트))
...
void CL_W10_WebAPI::routeStaticAssets() {
	...
	for (uint8_t v_i = 0; v_i < G_W10_PAGE_COUNT; v_i++) {
		...
		const char* v_html_file = v_page.html;
		const char* v_uri = v_page.uri;
		
		// 1-1. HTML 파일 라우팅 등록
		// 메인 페이지인 경우, 설정값 오버라이드 로직 적용
		if (strcmp(v_uri, "/") == 0) {
			const char* v_cfg_html = v_web.html;
			if (strlen(v_cfg_html) > 0)
				W10_pushRoute(v_cfg_html, v_cfg_html, "text/html"); // 설정 오버라이드 경로 등록
			W10_pushRoute(v_html_file, v_html_file, "text/html"); // 기본 파일 경로 등록
		} else {
			// 그 외 페이지는 단축 URI와 파일 경로 모두 등록
			W10_pushRoute(v_uri, v_html_file, "text/html");
			W10_pushRoute(v_html_file, v_html_file, "text/html");
		}

		// 1-2. 전용 CSS 파일 라우팅 등록
		if (v_page.css) {
			if (v_page.css[0] == '/') { // 절대 경로만 등록
				W10_pushRoute(v_page.css, v_page.css, "text/css");
			}
		}
		
		// 1-3. 전용 JS 파일 라우팅 등록
		if (v_page.js) {
			if (v_page.js[0] == '/') { // 절대 경로만 등록
				W10_pushRoute(v_page.js, v_page.js, "application/javascript");
			}
		}
	} // end for

	// 2. 공통 CSS/JS 파일 라우팅 등록 (모든 페이지에서 사용 가능하도록)
	...
	s_server->on("/", HTTP_GET, [](AsyncWebServerRequest* r) {
		...
		const char* v_default_html = s_pages_static[0].html; // 메인 페이지의 기본 HTML 경로 사용
		
		// 설정된 HTML 파일 경로가 있고 해당 파일이 LittleFS에 존재하면
		if (strlen(f) && LittleFS.exists(f))
			r->redirect(f); // 설정된 경로로 리다이렉트
		else
			r->redirect(v_default_html); // 기본 HTML 경로로 리다이렉트 (폴백)
	});
