...
class CL_D10_Logger {
   public:
	...
	static void applyCoreConfig(const ST_A10_SystemConfig& p_sys) {
		// level
		if (strcasecmp(p_sys.system.logging.level, "DEBUG") == 0)
			s_logLevel = EN_L10_LOG_DEBUG;
		else if (strcasecmp(p_sys.system.logging.level, "WARN") == 0)
			s_logLevel = EN_L10_LOG_WARN;
		else if (strcasecmp(p_sys.system.logging.level, "ERROR") == 0)
