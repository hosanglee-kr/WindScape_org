...
void CL_C10_ConfigManager::saveDirtyConfigs() {
    ...
    if (_dirty_system && g_A10_config_root.system) {
        if (saveSystemConfig(*g_A10_config_root.system))
            _dirty_system = false;
    }
    if (_dirty_wifi && g_A10_config_root.wifi) {
        if (saveWifiConfig(*g_A10_config_root.wifi))
            _dirty_wifi = false;
    }
    if (_dirty_motion && g_A10_config_root.motion) {
        if (saveMotionConfig(*g_A10_config_root.motion))
            _dirty_motion = false;
    }
    if (_dirty_schedules && g_A10_config_root.schedules) {
        if (saveSchedules(*g_A10_config_root.schedules))
            _dirty_schedules = false;
    }
    if (_dirty_userProfiles && g_A10_config_root.userProfiles) {
        if (saveUserProfiles(*g_A10_config_root.userProfiles))
            _dirty_userProfiles = false;
    }
    if (_dirty_windProfile && g_A10_config_root.windDict) {
        if (saveWindProfileDict(*g_A10_config_root.windDict))
