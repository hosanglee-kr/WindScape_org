...
// - 자연풍 “미세 불규칙성”을 만들되, 너무 빠른 루프/부하를 막기 위함
static const uint32_t G_S10_TICK_MIN_BASE_MS    = 40u;
static const uint32_t G_S10_TICK_JITTER_RANGE_MS = 60u;

// 돌풍/열기포 “발생 여부 평가” 최소 간격
// - 확률은 평가 주기에 의존하므로, 평가 주기 변경 시 확률 보정 로직과 반드시 짝으로 관리
static const uint32_t G_S10_GUST_EVAL_MIN_MS    = 500u;
static const uint32_t G_S10_THERM_EVAL_MIN_MS   = 700u;

// 차트 샘플링 간격 정책
static const uint32_t G_S10_CHART_HZ1_MS        = 1000u; // 평상시 1Hz
static const uint32_t G_S10_CHART_HZ2_MS        = 500u;  // 이벤트 중 2Hz

// 차트 Full Dump 최소 전송 간격 (Web 부하 제어)
static const uint32_t G_S10_CHART_FULL_MIN_MS   = 10000u; // 10s
