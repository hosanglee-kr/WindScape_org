...
bool CL_WF10_WiFiManager::startAP(const ST_A10_WifiConfig& p_cfg_wifi,
	...
	bool v_ok =
		WiFi.softAP(p_cfg_wifi.ap.ssid, v_pass, p_channel, false, 4);
...
void WF10_applyTimeConfigFromSystem(const ST_A10_SystemConfig& p_cfg) {
	...
	CL_D10_Logger::log(
		EN_L10_LOG_INFO,
		"[WF10] Apply time config: ntp=%s, tz=%s, interval=%u min",
	...
	if (v_ok) {
		...
		CL_D10_Logger::log(
			EN_L10_LOG_WARN,
			"[WF10] Time sync not yet complete after config apply (non-fatal)"
		);
	}
