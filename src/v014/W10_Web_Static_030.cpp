...
/*
 * ------------------------------------------------------
 * 소스명 : W10_Web_Static_030.cpp
 * 모듈 약어 : W10
 * 모듈명 : Smart Nature Wind Web API (v029) - Static Assets & Menu Implementation
 * ------------------------------------------------------
 * 기능 요약:
 * - LittleFS 기반 JSON 파일(`/json/cfg_pages_029.json`)에서 웹 페이지(`pages`)와 정적 자산(`assets`) 정보 로드.
...
// JSON 파일 경로 (v029)
constexpr char G_W10_PAGES_JSON[] = "/json/cfg_pages_029.json";
...
//  - cfg_pages_029.json의 pages를 order순으로 정렬 후,
//    isMain == false 인 항목만 반환
//  - path는 JSON에 정의된 값을 그대로 사용 (가공 없음)
// ------------------------------------------------------
