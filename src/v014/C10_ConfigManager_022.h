...
class CL_C10_ConfigManager {
   public:
	...
	static bool loadSystemConfig(ST_A10_SystemConfig& p_cfg) {
		JsonDocument v_doc;
		if (!ioLoadJson(A10_Const::CFG_SYSTEM_FILE, v_doc)) {
	...
	static bool loadWifiConfig(ST_A10_WifiConfig& p) {
		JsonDocument d;
		if (!ioLoadJson(A10_Const::CFG_WIFI_FILE, d)) {
	...
	static bool loadMotionConfig(ST_A10_MotionConfig& p) {
		JsonDocument d;
		if (!ioLoadJson(A10_Const::CFG_MOTION_FILE, d)) {
	...
	static bool loadWindProfileDict(ST_A10_WindProfileDict_t& p_dict) {
		JsonDocument d;
		if (!ioLoadJson(A10_Const::CFG_WIND_PROFILE_FILE, d)) {
		...
		if (j["presets"].is<JsonArrayConst>()) {
			...
			for (JsonObjectConst v_js : v_arr) {
				if (p_dict.presetCount >= A10_Const::MAX_WIND_PRESETS)
					break;
		...
		if (j["styles"].is<JsonArrayConst>()) {
			...
			for (JsonObjectConst v_js : v_arr) {
				if (p_dict.styleCount >= A10_Const::MAX_WIND_STYLES)
					break;
	...
	// =====================================================
	static bool loadSchedules(ST_A10_ScheduleConfig& p_cfg) {
		JsonDocument d;
		if (!ioLoadJson(A10_Const::CFG_SCHEDULES_FILE, d)) {
		...
		for (JsonObjectConst js : arr) {
			...
			if (js["segments"].is<JsonArrayConst>()) {
				...
				for (JsonObjectConst jseg : segArr) {
					if (s.segCount >= A10_Const::MAX_SEGMENTS_PER_SCHEDULE)
						break;
	...
	static bool saveSchedules(const ST_A10_ScheduleConfig& p_cfg) {
		JsonDocument d;
		...
		return ioSaveJson(A10_Const::CFG_SCHEDULES_FILE, A10_Const::CFG_SCHEDULES_FILE_BAK, d);
	}

	static void toJson_Schedules(const ST_A10_ScheduleConfig& p, JsonDocument& d) {
	...
	// =====================================================
	static bool loadUserProfiles(ST_A10_UserProfileConfig_t& p_cfg) {
		JsonDocument d;
		if (!ioLoadJson(A10_Const::CFG_USER_PROFILES_FILE, d)) {
		...
		for (JsonObjectConst jp : arr) {
			...
			if (jp["segments"].is<JsonArrayConst>()) {
				...
				for (JsonObjectConst jseg : sArr) {
					if (up.segCount >= A10_Const::MAX_SEGMENTS_PER_PROFILE)
						break;
	...
	static bool saveUserProfiles(const ST_A10_UserProfileConfig_t& p_cfg) {
		JsonDocument d;
		for (uint8_t v_i = 0; v_i < p_cfg.count; v_i++) {
		...
		return ioSaveJson(A10_Const::CFG_USER_PROFILES_FILE, A10_Const::CFG_USER_PROFILES_FILE_BAK, d);
	}

	static void toJson_UserProfiles(const ST_A10_UserProfileConfig_t& p, JsonDocument& d) {
...
static void freeLazySection(const char* p_section, ST_A10_ConfigRoot& p_root) {
	...
	// 메모리 해제 (동적 섹션 정리)
	// =====================================================
	static void freeAll(ST_A10_ConfigRoot& p_root) {
		if (p_root.wifi) {
			delete p_root.wifi;
			p_root.wifi = nullptr;
		}
