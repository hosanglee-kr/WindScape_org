...
class CL_CT10_ControlManager {
   private:
	// ✅ 3.A Magic Number 상수화 적용
	static constexpr uint32_t S_TICK_MIN_INTERVAL_MS	  = 40UL;
	static constexpr uint32_t S_METRICS_PUSH_INTERVAL_MS = 1500UL;
   ...
   public:
	...
	void _tick() {
		...
		// ✅ 1. Dirty 플래그 기반 자동 브로드캐스트 로직 제거
		// SC10_run()에서 외부적으로 Dirty 플래그를 확인하고 처리함
	}
	...
	// --------------------------------------------------
	// Dirty 플래그 관리 (SC10 diffOnly 연동용 - 1. 적용)
	// --------------------------------------------------
   ...
   private:
	...
	bool _tickOverride() {
		...
		if (!overrideState.resolvedApplied) {
			...
			// ✅ override 적용 직후 차트 갱신 플래그 설정 (1. 적용)
			markDirty("chart");
		}
...
void _applySegmentOn(const T_segment& p_seg) {
	...
	if (v_ok && v_res.valid) {
		...
		// ✅ Segment 변경 시 상태 브로드캐스트 플래그 설정 (1. 적용)
		markDirty("state");
		markDirty("chart");
	} else {
		CL_D10_Logger::log(EN_L10_LOG_WARN,
	...
	// --------------------------------------------------
	// 유틸리티 함수 (3.C 적용: static으로 독립성 확보)
	// --------------------------------------------------
	// ✅ 3.C 적용: static으로 변경
	...
	// 상태 변경 시 WebSocket 브로드캐스트 헬퍼 (1. 적용: 브로드캐스트 로직 제거됨)
	// --------------------------------------------------
	...
	// --------------------------------------------------
	// Metrics WebSocket 브로드캐스트 (주기 + diffOnly) (1. 적용: SC10에 위임)
	// --------------------------------------------------
	void _maybeBroadcastMetrics() {
		...
		// => markDirty("metrics")로 대체됨 (tick()에서 최종 브로드캐스트 로직은 제거됨)
		markDirty("metrics");
	}
};
