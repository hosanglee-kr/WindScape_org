 ...
 * ------------------------------------------------------
 * [구현 규칙]
 *  - ArduinoJson v7.x.x, JsonDocument 단일 타입만 사용
 *  - createNestedArray/Object/containsKey 사용 금지
 *  - memset + strlcpy 기반 안전 초기화
 *  - 주석/필드명은 JSON 구조와 동일하게 유지
 *  - 단일 헤더(h) 파일 구성 (cpp 없음)
 * ------------------------------------------------------
 * [코드 네이밍 규칙]
 *  - 전역 상수/매크로 : G_B10_*
 *  - 전역 변수        : g_B10_*
...
public:
  ...
  // ===== 결과 조회 & 직렬화 =====
  bool isAnyPresent() const { return _global.anyPresent; }
  EN_B10_presence_state_t getPresenceState() const { return _global.presenceState; }

  void toJson(JsonDocument& p_doc) const {
    JsonObject b = p_doc["ble"].to<JsonObject>();
    b["any_present"]  = _global.anyPresent;
    b["state"]        = (_global.presenceState==EN_B10_PRESENCE_IDLE ? "IDLE" :
                         _global.presenceState==EN_B10_PRESENCE_DETECT ? "DETECT" : "EXIT_DELAY");
    b["since_ms"]     = _global.stateSinceMs;
    b["on"]           = _rssiCfg.on;
    b["off"]          = _rssiCfg.off;
    b["avg_count"]    = _rssiCfg.avg_count;
    b["persist_count"]= _rssiCfg.persist_count;
    b["exit_delay_s"] = _rssiCfg.exit_delay_sec;
    b["scan_interval_s"] = _scanIntervalSec;

    JsonArray arr = b["trusted_devices"].to<JsonArray>();
    for (uint8_t v_i=0; v_i<_trustedCount; ++v_i) {
      JsonObject t = arr.add<JsonObject>();
      t["alias"] = _trusted[v_i].alias;
      t["name"]  = _trusted[v_i].name;
      t["mac"]   = _trusted[v_i].mac;
      t["manuf_prefix"] = _trusted[v_i].manuf_prefix;
      t["prefix_len"]   = _trusted[v_i].prefix_len;
      t["enabled"]      = _trusted[v_i].enabled;
    }

    JsonArray s = b["devices"].to<JsonArray>();
    for (uint8_t v_i=0; v_i<_trustedCount; ++v_i) {
      const ST_B10_RuntimeDev_t& r = _rt[v_i];
      JsonObject d = s.add<JsonObject>();
      d["alias"]       = _trusted[v_i].alias;
      d["present"]     = r.present;
      d["seen"]        = r.seenRecently;
      d["last_ms"]     = r.lastSeenMs;
      d["rssi"]        = r.lastRssi;
      d["samp_count"]  = r.sampCount;
      d["persist_on"]  = r.persistOn;
      d["persist_off"] = r.persistOff;
    }
  }

  // ===== 런타임 설정(CT10에서 동적 조정) =====
  void setOnThreshold(int p_val){ _rssiCfg.on  = p_val; }
  void setOffThreshold(int p_val){ _rssiCfg.off = p_val; }
  void setAvgCount(uint8_t p_n)  { _rssiCfg.avg_count = (p_n>0 && p_n<=G_B10_MAX_SAMPLES_PER_DEV)?p_n:_rssiCfg.avg_count; }
  void setPersistCount(uint8_t p_n){ _rssiCfg.persist_count = (p_n>0)?p_n:_rssiCfg.persist_count; }
  void setExitDelay(uint16_t p_s){ _rssiCfg.exit_delay_sec = (p_s>0)?p_s:_rssiCfg.exit_delay_sec; }
  void setScanInterval(uint16_t p_s){ _scanIntervalSec = (p_s>0)?p_s:_scanIntervalSec; }

  // 설정 재적용(파일 변경 후)
  void reloadConfig(){ _loadConfig(); _resetRuntime(); }
...
private:
  ...
  void _loadConfig(){
    ...
    _scanIntervalSec = (g_A10_config_root.core.hw.ble.enabled && g_A10_config_root.core.hw.ble.scan_interval>0)
                        ? g_A10_config_root.core.hw.ble.scan_interval
    ...
    if (g_A10_config_root.motion){
      ...
      if (g_A10_config_root.motion->ble.rssi_on)   _rssiCfg.on  = g_A10_config_root.motion->ble.rssi_on;
      if (g_A10_config_root.motion->ble.rssi_off)  _rssiCfg.off = g_A10_config_root.motion->ble.rssi_off;
      if (g_A10_config_root.motion->ble.avg_count) _rssiCfg.avg_count = g_A10_config_root.motion->ble.avg_count;
      if (g_A10_config_root.motion->ble.persist_count) _rssiCfg.persist_count = g_A10_config_root.motion->ble.persist_count;
      if (g_A10_config_root.motion->ble.exit_delay_sec) _rssiCfg.exit_delay_sec = g_A10_config_root.motion->ble.exit_delay_sec;
    }

    // 히스테리시스 보정: on > off 되도록 강제
    if (_rssiCfg.on <= _rssiCfg.off) _rssiCfg.on = _rssiCfg.off + 5;
  }
  ...
  void _consumeAdv(const char* p_name, const char* p_mac, int p_rssi, const char* p_manufHex, unsigned long p_nowMs){
    ...
    if (v_r.sampCount < _rssiCfg.avg_count){
      ...
      if (v_r.idx >= _rssiCfg.avg_count) v_r.idx = 0;
      v_r.samples[v_r.idx++] = p_rssi;
    }
    ...
    if (v_avg >= _rssiCfg.on){
      if (v_r.persistOn < 255) v_r.persistOn++;
      v_r.persistOff = 0;
      ...
      if (v_r.persistOff < 255) v_r.persistOff++;
      v_r.persistOn = 0;
      ...
      if (v_r.persistOn>0)  v_r.persistOn--;
      if (v_r.persistOff>0) v_r.persistOff--;
    }
  }
  ...
  void _tickPresenceFSM(){
    ...
    for (uint8_t v_i=0; v_i<_trustedCount; ++v_i){
      ...
      // 전역 현재 근접?
      if (v_r.present) v_anyPresent = true;
    }
  ...
  int _matchTrusted(const char* p_name, const char* p_mac, const char* p_manufHex){
    // 표준화
    char v_mac[18] = {0};
    if (p_mac && p_mac[0]){ strlcpy(v_mac, p_mac, sizeof(v_mac)); _toUpperHexMac(v_mac); }
    ...
    for (uint8_t v_i=0; v_i<_trustedCount; ++v_i){
      ...
      // 1) MAC 일치
      if (t.mac[0] && v_mac[0] && _macEqual(t.mac, v_mac)) v_match = true;

      // 2) 이름 매칭(정확히 같음)
      if (!v_match && t.name[0] && p_name && p_name[0] && strcmp(t.name, p_name)==0) v_match = true;

      // 3) Manufacturer Prefix(HEX) 매칭
      if (!v_match && t.manuf_prefix[0] && p_manufHex && p_manufHex[0]){
        if (_hexPrefixMatch(t.manuf_prefix, t.prefix_len, p_manufHex)) v_match = true;
      }

      if (v_match) return (int)v_i;
    }
    return -1;
  }
  ...
  static int _avgRssi(const ST_B10_RuntimeDev_t& r){
    if (r.sampCount==0) return r.lastRssi;
    long v_sum = 0;
  ...
  static bool _macEqual(const char* a, const char* b){ return strncmp(a,b,17)==0; }
  ...
  static void _binToHex(const uint8_t* p_bin, uint8_t p_len, char* p_out, size_t p_outLen){
    ...
    if (v_pos<p_outLen) p_out[v_pos]=0;
  }
