...
class CL_CT10_ControlManager {
   ...
   public:
	...
	void toJson(JsonDocument& p_doc) {
		...
		ov["remainSec"] = overrideState.active ? (int32_t)(overrideState.until_sec - (millis() / 1000UL)) : 0;
   ...
   private:
	...
	void _applyContinuousPreset() {
		if (!sim || !g_A10_config_root.control)
			return;
	...
	// ==================================================
	void _applyOverride() {
		if (!pwm || !sim || !overrideState.active)
			return;
		if (overrideState.overrideMode == EN_CT10_OVERRIDE_FIXED) {
			sim->S10_stop();
	...
	bool _isMotionGateActive() {
		if (!motion || !g_A10_config_root.control)
			return false;
		const auto& v_m = g_A10_config_root.control->Continuous.motion;
		if (!v_m.pir.enabled && !v_m.ble.enabled)
			return false;
	...
	bool _isMotionGateActiveForSchedule(const decltype((*g_A10_config_root.control).schedules[0])& p_sch) {
		if (!motion)
			return false;
		if (!p_sch.motion.pir.enabled && !p_sch.motion.ble.enabled)
			return false;
		extern bool M10_motionDetected();
		return !M10_motionDetected();
	}
	...
	int _findActiveSchedule(const decltype((*g_A10_config_root.control).schedules)& p_arr) {
		...
		for (size_t i = 0; i < p_arr.size(); ++i) {
			...
			if (!v_s.enabled)
				continue;
			if (v_s.days.size() == 7 && v_s.days[v_wday] == 0)
				continue;
			int v_st = _parseHHMM(v_s.start_time);
			int v_ed = _parseHHMM(v_s.end_time);
			if (v_st < 0 || v_ed < 0)
				continue;
			if (v_st <= v_nowMin && v_nowMin < v_ed)
				return (int)i;
			if (v_ed < v_st && (v_nowMin >= v_st || v_nowMin < v_ed))
				return (int)i;
		}
		return -1;
	}
