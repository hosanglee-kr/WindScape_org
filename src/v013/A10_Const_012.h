...
inline void A10_resetSystemDefault(ST_A10_SystemConfig& s) {
	...
	s.hw.fan_pwm.res	  = 10;
	s.hw.pir.enabled	  = true;
	s.hw.pir.pin		  = 13;
	s.hw.pir.debounce_sec = 5;
	s.hw.tempHum.enabled  = true;
	strlcpy(s.hw.tempHum.type, "DHT22", sizeof(s.hw.tempHum.type));
	s.hw.tempHum.pin		  = 23;
	s.hw.tempHum.interval_sec = 30;
	s.hw.ble.enabled		  = true;
	s.hw.ble.scan_interval	  = 5;
