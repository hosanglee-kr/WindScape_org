...
public:
  ...
  bool getPresetIndexByName(const char* p_name, int &p_idx) {
    for (int i=0;i<SC10_PRESET_COUNT;i++) {
      if (strcmp(p_name, G_SC10_PRESET_MODE_NAMES[i])==0){ p_idx=i; return true; }
    }
    return false;
  }
  ...
  void applyCurrentPreset(bool p_force_apply) {
    ...
    gust_active = false; thermal_bubble_active = false; gust_intensity = 1.0f;
    ...
    if (v_preset == SC10_PRESET_OFF) {
      ...
      current_wind_speed = 0.5f; target_wind_speed = 0.5f;
    } else {
      // 환경값 스위치
      switch(v_preset){
        case SC10_PRESET_COUNTRY:       base_wind_min=0.7f; base_wind_max=3.4f; gust_probability_base=0.006f; location_gust_strength=1.35f; thermal_bubble_frequency=0.015f; break;
        case SC10_PRESET_MEDITERRANEAN: base_wind_min=1.6f; base_wind_max=3.8f; gust_probability_base=0.012f; location_gust_strength=1.55f; thermal_bubble_frequency=0.035f; break;
        case SC10_PRESET_OCEAN:         base_wind_min=1.8f; base_wind_max=5.5f; gust_probability_base=0.040f; location_gust_strength=2.1f;  thermal_bubble_frequency=0.022f; break;
        case SC10_PRESET_MOUNTAIN:      base_wind_min=2.2f; base_wind_max=7.5f; gust_probability_base=0.045f; location_gust_strength=2.2f;  thermal_bubble_frequency=0.028f; break;
        case SC10_PRESET_PLAINS:        base_wind_min=4.0f; base_wind_max=8.8f; gust_probability_base=0.070f; location_gust_strength=2.4f;  thermal_bubble_frequency=0.018f; break;
        default:                        base_wind_min=1.8f; base_wind_max=5.5f; gust_probability_base=0.040f; location_gust_strength=2.1f;  thermal_bubble_frequency=0.022f; break;
      }
      startWindSimulation();
    }
  }
  ...
  void startWindSimulation() {
    ...
    wind_momentum=0.0f; spectral_energy_buffer=0.0f; spectral_phase_accumulator=0.0f;

    current_weather_phase = SC10_WEATHER_PHASE_NORMAL;
    phase_start_time = millis()/1000.0f; phase_duration = 120.0f;
  ...
  void applyFanSpeed(float p_speed_percent) {
    ...
    if (v_intensity <= 0.01f) { ledcWrite(g_SC10_config.pwm_channel, 0); return; }
    if (wind_simulation_active) v_req *= v_intensity;
    v_req = fmax(v_min, fmin(v_limit, v_req));

    int levels = (1 << g_SC10_config.pwm_resolution) - 1;
    int pwm = (int)(v_req * levels);
    if (v_req <= 0.01f) pwm = 0;
  ...
  void calculateVonKarman(float p_dt) {
    ...
    if (spectral_phase_accumulator > 2.0f * M_PI) spectral_phase_accumulator -= 2.0f * M_PI;
  ...
  void calculateThermal() {
    ...
    if (age >= thermal_bubble_duration) { thermal_bubble_active=false; return; }
  ...
  void updateGust() {
    ...
    if (gust_active) {
      ...
      if (age >= gust_duration) { gust_active=false; gust_intensity=1.0f; }
      else {
        float prog = age / gust_duration;
        float env;
        if (prog < 0.25f) env = 1.0f - powf(1.0f - prog/0.25f, 1.8f);
        else if (prog < 0.65f) { env = 1.0f; float osc = 1.5f + (current_weather_phase * 0.5f); env += sinf(age*osc)*0.08f; }
        else { float d=(prog-0.65f)/0.35f; env = 1.0f - powf(d, 1.5f); }
    ...
    if ((millis() - last_gust_check) >= (unsigned long)g_SC10_config.gust_check_interval_ms) {
      ...
      if (current_weather_phase == SC10_WEATHER_PHASE_CALM) phase_mul = 0.3f * wind_factor;
      else if (current_weather_phase == SC10_WEATHER_PHASE_STRONG) phase_mul = 2.2f * wind_factor;
      else phase_mul = 0.9f * wind_factor;
  ...
  void updateThermalCheck() {
    ...
    if ((millis() - last_thermal_check) < (unsigned long)g_SC10_config.thermal_check_interval_ms) return;
    last_thermal_check = millis();
    float base = thermal_bubble_frequency;
    float wind_factor = 1.0f + (current_wind_speed / 8.0f) * 0.3f;
    float phase_mul = (current_weather_phase==SC10_WEATHER_PHASE_CALM)?1.2f:
                      (current_weather_phase==SC10_WEATHER_PHASE_STRONG)?0.7f:1.0f;
    float p = base * wind_factor * phase_mul;
    if (SC10_getRandom01() < p) {
      thermal_bubble_active = true; thermal_bubble_start_time = millis()/1000.0f;
      float dur = SC10_randRange(8.0f, 14.0f);
      if (current_weather_phase == SC10_WEATHER_PHASE_CALM) dur *= 1.3f;
      else if (current_weather_phase == SC10_WEATHER_PHASE_STRONG) dur *= 0.8f;
  ...
  void updateWeatherPhase() {
    ...
    if (old == SC10_WEATHER_PHASE_CALM) {
      ...
      if (r<0.4f) current_weather_phase = SC10_WEATHER_PHASE_CALM;
      else if (r<0.8f) current_weather_phase = SC10_WEATHER_PHASE_NORMAL;
    ...
    if (current_weather_phase == SC10_WEATHER_PHASE_CALM) {
      ...
      phase_wind_min = base_wind_min; phase_wind_max = base_wind_min + span*0.6f;
    } else if (current_weather_phase == SC10_WEATHER_PHASE_NORMAL) {
      phase_duration = SC10_randRange(120.0f, 300.0f);
      phase_wind_min = base_wind_min + span*0.15f; phase_wind_max = base_wind_min + span*0.85f;
    } else {
      phase_duration = SC10_randRange(60.0f, 150.0f);
      phase_wind_min = base_wind_min + span*0.4f; phase_wind_max = base_wind_max;
    }
  ...
  void generateWindTarget() {
    ...
    if (current_weather_phase==SC10_WEATHER_PHASE_CALM) base_rate = 0.08f + var*0.12f;
    else if (current_weather_phase==SC10_WEATHER_PHASE_STRONG) base_rate = 0.25f + var*0.35f;
    else base_rate = 0.15f + var*0.25f;

    float U = current_wind_speed; if (U<0.1f) U=0.1f;
  ...
  void tick() {
    ...
    float far_ch    = 6.0f;
    if (g_SC10_config.wind_variability > 70.0f) { close_ch*=1.5f; far_ch*=1.5f; }
    if (fabsf(v_diff) < change_th) {
      if (SC10_randRange(0.0f,100.0f) < close_ch) generateWindTarget();
