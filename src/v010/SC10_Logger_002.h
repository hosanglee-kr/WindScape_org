...
typedef enum { SC10_LOG_DEBUG, SC10_LOG_INFO, SC10_LOG_WARN, SC10_LOG_ERROR } SC10_LogLevel;

class SC10_Logger {
public:
  static void setLevel(SC10_LogLevel p_lvl) { g_logLevel = p_lvl; }

  static void log(SC10_LogLevel p_lvl, const char* fmt, ...) {
    if (p_lvl < g_logLevel) return;
    char buf[256];
...
private:
  ...
  static const char* levelStr(SC10_LogLevel l) {
    switch(l){case SC10_LOG_DEBUG:return "DEBUG";case SC10_LOG_INFO:return "INFO";
      case SC10_LOG_WARN:return "WARN";case SC10_LOG_ERROR:return "ERROR";}
    return "";
  }
  static void push(const String &s) {
    if (g_logs.size() > 100) g_logs.pop_front();
    g_logs.push_back(s);
  }
};
