...
public:
    ...
    static bool patchFromJson(WindConfig &p_cfg, const JsonDocument &p_doc, bool &p_wifiChanged) {
        ...
        // --- 시뮬레이션 파라미터 ---
        if (!v_root["intensity"].isNull())    p_cfg.wind_intensity = v_root["intensity"].as<float>();
        if (!v_root["gust_freq"].isNull())    p_cfg.gust_frequency = v_root["gust_freq"].as<float>();
        if (!v_root["variability"].isNull())  p_cfg.wind_variability = v_root["variability"].as<float>();
        if (!v_root["fan_limit"].isNull())    p_cfg.fan_speed_limit = v_root["fan_limit"].as<float>();
        if (!v_root["min_fan"].isNull())      p_cfg.minimum_fan_speed = v_root["min_fan"].as<float>();
        if (!v_root["turb_len"].isNull())     p_cfg.turbulence_length_scale = v_root["turb_len"].as<float>();
        if (!v_root["turb_sig"].isNull())     p_cfg.turbulence_intensity_sigma = v_root["turb_sig"].as<float>();
        if (!v_root["therm_str"].isNull())    p_cfg.thermal_bubble_strength = v_root["therm_str"].as<float>();
        if (!v_root["therm_rad"].isNull())    p_cfg.thermal_bubble_radius = v_root["therm_rad"].as<float>();

        // --- 타이밍 ---
        if (!v_root["sim_int"].isNull())      p_cfg.wind_sim_interval_ms = v_root["sim_int"].as<int>();
        if (!v_root["gust_int"].isNull())     p_cfg.gust_check_interval_ms = v_root["gust_int"].as<int>();
        if (!v_root["thermal_int"].isNull())  p_cfg.thermal_check_interval_ms = v_root["thermal_int"].as<int>();

        // --- Wi-Fi ---
        if (!v_root["wifi_mode"].isNull()) { p_cfg.wifi_mode = v_root["wifi_mode"].as<int>(); p_wifiChanged = true; }
        if (!v_root["ap_ssid"].isNull())   { strlcpy(p_cfg.ap_ssid, v_root["ap_ssid"], sizeof(p_cfg.ap_ssid)); p_wifiChanged = true; }
        if (!v_root["ap_password"].isNull()){ strlcpy(p_cfg.ap_password, v_root["ap_password"], sizeof(p_cfg.ap_password)); p_wifiChanged = true; }
...
private:
    ...
    static bool parseJson(WindConfig &p_c, JsonDocument &p_doc) {
        ...
        for (JsonObjectConst v_net : v_arr) {
            if (p_c.sta_network_count >= SC10_Const::MAX_STA_NETWORKS) break;
            const char* v_ssid = v_net["ssid"] | "";
            const char* v_pass = v_net["pass"] | "";
        ...
        // 존재하지 않으면 기존값 유지
        if (!v_root["sim"]["intensity"].isNull())     p_c.wind_intensity = v_root["sim"]["intensity"].as<float>();
        if (!v_root["sim"]["gust_freq"].isNull())     p_c.gust_frequency = v_root["sim"]["gust_freq"].as<float>();
        if (!v_root["sim"]["variability"].isNull())   p_c.wind_variability = v_root["sim"]["variability"].as<float>();
        if (!v_root["sim"]["fan_limit"].isNull())     p_c.fan_speed_limit = v_root["sim"]["fan_limit"].as<float>();
        if (!v_root["sim"]["min_fan"].isNull())       p_c.minimum_fan_speed = v_root["sim"]["min_fan"].as<float>();
        if (!v_root["sim"]["turb_len"].isNull())      p_c.turbulence_length_scale = v_root["sim"]["turb_len"].as<float>();
        if (!v_root["sim"]["turb_sig"].isNull())      p_c.turbulence_intensity_sigma = v_root["sim"]["turb_sig"].as<float>();
        if (!v_root["sim"]["therm_str"].isNull())     p_c.thermal_bubble_strength = v_root["sim"]["therm_str"].as<float>();
        if (!v_root["sim"]["therm_rad"].isNull())     p_c.thermal_bubble_radius = v_root["sim"]["therm_rad"].as<float>();

        if (!v_root["timing"]["sim_int"].isNull())     p_c.wind_sim_interval_ms = v_root["timing"]["sim_int"].as<int>();
        if (!v_root["timing"]["gust_int"].isNull())    p_c.gust_check_interval_ms = v_root["timing"]["gust_int"].as<int>();
        if (!v_root["timing"]["thermal_int"].isNull()) p_c.thermal_check_interval_ms = v_root["timing"]["thermal_int"].as<int>();
