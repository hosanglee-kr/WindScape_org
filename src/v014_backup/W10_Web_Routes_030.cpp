...
// 초기화
// --------------------------------------------------
void                    CL_W10_WebAPI::begin(AsyncWebServer&         p_server,
                                             CL_CT10_ControlManager& p_control,
                                             WiFiMulti&              p_multi) {
    s_server  = &p_server;
    s_control = &p_control;
    s_multi   = &p_multi;
...
void CL_W10_WebAPI::routeSystem() {
    ...
    // POST (패치)
    s_server->on(W10_Const::HTTP_API_SYSTEM, HTTP_POST, [](AsyncWebServerRequest* p_request) {}, nullptr, [](AsyncWebServerRequest* p_request, uint8_t* p_data, size_t p_len, size_t p_index, size_t p_total) {
					 if (!checkApiKey(p_request)) {
						 p_request->send(401, "application/json", "{\"error\":\"unauthorized\"}");
						 return;
					 }
                    ...
                    size_t p_len, size_t p_index, size_t p_total) {
                     ...
                     if (v_changed) {
                         ...
                         CL_D10_Logger::log(EN_L10_LOG_INFO,
                                            "[W10] WiFi config patched & applied via /api/wifi");
                     }
...
void CL_W10_WebAPI::routeMotion() {
    ...
    // POST
    s_server->on(W10_Const::HTTP_API_MOTION, HTTP_POST, [](AsyncWebServerRequest* p_request) {}, nullptr, [](AsyncWebServerRequest* p_request, uint8_t* p_data, size_t p_len, size_t p_index, size_t p_total) {
					 if (!checkApiKey(p_request)) {
						 p_request->send(401, "application/json", "{\"error\":\"unauthorized\"}");
						 return;
					 }
    ...
    s_server->on(
        ...
        [](AsyncWebServerRequest* p_request, uint8_t* p_data,
           size_t p_len, size_t p_index, size_t p_total) {
            ...
            bool v_updated = CL_C10_ConfigManager::updateWindProfileFromJson(
                (uint16_t)v_id, v_doc);
    ...
    s_server->on(
        ...
        [](AsyncWebServerRequest* p_request, uint8_t* p_data,
           size_t p_len, size_t p_index, size_t p_total) {
            ...
            JsonDocument v_res;
            if (v_id > 0 && s_control &&
                s_control->startUserProfileByNo((uint8_t)v_id)) {
    ...
    s_server->on(
        ...
        [](AsyncWebServerRequest* p_request) {
            ...
            CL_D10_Logger::log(EN_L10_LOG_WARN,
                               "[W10] Config Init requested via /api/config/init.");
            bool         v_ok = CL_C10_ConfigManager::factoryResetFromDefault();
