...
bool ioLoadJson(const char* p_path, JsonDocument& p_doc) {
	...
	p_doc.clear();
	{
		...
		DeserializationError v_eb = deserializeJson(p_doc, v_fb);
		v_fb.close();
	...
	// 4) bak -> main 복원 (p_doc는 이미 bak 기준으로 유효)
...
bool CL_C10_ConfigManager::_loadCfgJsonFile() {
	...
	CL_D10_Logger::log(EN_L10_LOG_INFO,
	                   "[C10] cfg_jsonFile loaded: sys=%s wifi=%s motion=%s nvsSpec=%s sch=%s userProfiles=%s windDict=%s webPage=%s",
...
void CL_C10_ConfigManager::freeLazySection(const char* p_section, ST_A20_ConfigRoot_t& p_root) {
	...
	if (strcmp(p_section, "windDict") == 0 && p_root.windDict) {
	...
	if (strcmp(p_section, "nvsSpec") == 0 && p_root.nvsSpec) {
		delete p_root.nvsSpec;
		p_root.nvsSpec = nullptr;
		return;
	}
	if (strcmp(p_section, "webPage") == 0 && p_root.webPage) {
...
void CL_C10_ConfigManager::saveDirtyConfigs() {
	...
	if (_dirty_windDict && g_A20_config_root.windDict) {
		if (saveWindProfileDict(*g_A20_config_root.windDict)) _dirty_windDict = false;
	}
...
void CL_C10_ConfigManager::toJson_All(const ST_A20_ConfigRoot_t& p,
	...
	CL_D10_Logger::log(EN_L10_LOG_DEBUG,
	                   "[C10] Config export → JSON (sys=%d wifi=%d motion=%d sch=%d up=%d wind=%d nvs=%d web=%d)",
	                   p_includeSystem,
	                   p_includeWifi,
	                   p_includeMotion,
	                   p_includeSchedules,
...
// -----------------------------------------------------
// 8. Factory Reset 구현
//  - 이 코드는 프로젝트별 조건부 매크로/상수에 의존하므로
//    기존 정책 유지(필요 시 별도 파일에서 정리)
// -----------------------------------------------------
