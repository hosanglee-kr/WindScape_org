...
bool CL_C10_ConfigManager::_loadCfgJsonFile() {
    ...
    CL_D10_Logger::log(
        ...
        s_cfgJsonFileMap.uzOpProfile,
        s_cfgJsonFileMap.dft_windProfile);
...
void CL_C10_ConfigManager::saveDirtyConfigs() {
    ...
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] All dirty configs saved to storage.");
...
void CL_C10_ConfigManager::toJson_All(
    ...
    CL_D10_Logger::log(
        ...
        p_includeSystem, p_includeWifi,
        p_includeMotion, p_includeSchedules,
        p_includeUserProfiles);
}
...
bool CL_C10_ConfigManager::factoryResetFromDefault() {
    ...
    JsonDocument v_def;
    if (!ioLoadJson(A20_Const::CFG_DEFAULT_FILE,
                    A20_Const::CFG_DEFAULT_FILE_BAK,
                    v_def)) {
        CL_D10_Logger::log(
            EN_L10_LOG_ERROR,
            "[C10] Default file missing: %s",
            A20_Const::CFG_DEFAULT_FILE);
        return false;
    }
    ...
    if (v_def["system"].is<JsonObjectConst>()) {
        ...
        ioSaveJson(A20_Const::CFG_SYSTEM_FILE,
                   A20_Const::CFG_SYSTEM_FILE_BAK,
                   v_sys);
    }
    if (v_def["wifi"].is<JsonObjectConst>()) {
        JsonDocument v_wifi;
        v_wifi["wifi"] = v_def["wifi"];
        ioSaveJson(A20_Const::CFG_WIFI_FILE,
                   A20_Const::CFG_WIFI_FILE_BAK,
                   v_wifi);
    }
    if (v_def["motion"].is<JsonObjectConst>()) {
        JsonDocument v_motion;
        v_motion["motion"] = v_def["motion"];
        ioSaveJson(A20_Const::CFG_MOTION_FILE,
                   A20_Const::CFG_MOTION_FILE_BAK,
                   v_motion);
    }
    if (v_def["schedules"].is<JsonArrayConst>()) {
        JsonDocument v_sch;
        v_sch["schedules"] = v_def["schedules"];
        ioSaveJson(A20_Const::CFG_SCHEDULES_FILE,
                   A20_Const::CFG_SCHEDULES_FILE_BAK,
                   v_sch);
    }
    if (v_def["userProfiles"].is<JsonObjectConst>()) {
        JsonDocument v_up;
        v_up["userProfiles"] = v_def["userProfiles"];
        ioSaveJson(A20_Const::CFG_USER_PROFILES_FILE,
                   A20_Const::CFG_USER_PROFILES_FILE_BAK,
                   v_up);
    }
