...
// CL_CT10_ControlManager g_A00_control; // *인스턴스 선언 유지*
...
void A00_init() {
	...
	if (!g_A20_config_root.system || !g_A20_config_root.wifi) {
        CL_D10_Logger::log(EN_L10_LOG_ERROR,
                           "[A00] Config root invalid (system or wifi is null).");
	...
	// 4. Wi-Fi 초기화
	const ST_A20_WifiConfig&   v_wifi	= *g_A20_config_root.wifi;
	const ST_A20_SystemConfig& v_sys 	= *g_A20_config_root.system;
...
void A00_run() {
	...
	if (v_ctrl.consumeDirtyState()) {
		...
		A00_broadcastState(v_doc, true);
            ...
            CL_D10_Logger::log(EN_L10_LOG_DEBUG,
                               "[A00] State broadcast at %u ms (Δ=%u)",
	...
	if (v_ctrl.consumeDirtyMetrics()) {
		...
		A00_broadcastMetrics(v_doc, true);
            ...
            uint32_t v_now_ms = millis();
            CL_D10_Logger::log(EN_L10_LOG_DEBUG,
                               "[A00] Metrics broadcast at %u ms (Δ=%u)",
	...
	if (v_ctrl.consumeDirtyChart()) {
		...
		A00_broadcastChart(v_doc, true);
            ...
            uint32_t v_now_ms = millis();
            CL_D10_Logger::log(EN_L10_LOG_DEBUG,
                               "[A00] Chart broadcast at %u ms (Δ=%u)",
