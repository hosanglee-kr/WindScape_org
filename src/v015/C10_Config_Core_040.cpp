...
bool CL_C10_ConfigManager::_loadCfgJsonFile() {
	...
	CL_D10_Logger::log(EN_L10_LOG_INFO, "[C10] cfg_jsonFile loaded: sys=%s wifi=%s sch=%s userProfiles=%s windDic=%s webPage=%s", s_cfgJsonFileMap.system, s_cfgJsonFileMap.wifi, s_cfgJsonFileMap.schedules, s_cfgJsonFileMap.userProfiles, s_cfgJsonFileMap.windDict, s_cfgJsonFileMap.webPage);
...
bool CL_C10_ConfigManager::loadAll(ST_A20_ConfigRoot_t& p_root) {
	...
	// 1) 섹션 객체 확보
	if (!p_root.system)
		p_root.system = new ST_A20_SystemConfig_t();
	if (!p_root.windDict)
	...
	if (!p_root.wifi)
		p_root.wifi = new ST_A20_WifiConfig_t();
	if (!p_root.motion)
		p_root.motion = new ST_A20_MotionConfig_t();
