 ...
 * ------------------------------------------------------
 * [구현 시 반영 사항]
 *  - 항상 소스 시작 주석 체계 유지 및 내용 update
 *  - 주석에 있는 네이밍 규칙 적용
 *  - arduinojson v7.x.x 버전 사용(v5, v6 사용 금지)
 *      - StaticJsonDocument, DynamicJsonDocument은 JsonDocument를 사용
 *      - createNestedArray(), createNestedObject(), containsKey() 사용 금지
 * 
 * [코드 네이밍 규칙]
