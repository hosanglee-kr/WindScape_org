/*
 * ------------------------------------------------------
 * 소스명 : C10_Config_Schedule_029.cpp
 * 모듈 약어 : C10
...
static void C10_fromJson_ScheduleItem(
    ...
    strlcpy(p_s.period.start_time,
            p_js["period"]["start_time"] | "00:00",
            sizeof(p_s.period.start_time));
    strlcpy(p_s.period.end_time,
            p_js["period"]["end_time"] | "23:59",
            sizeof(p_s.period.end_time));
    ...
    if (p_js["autoOff"].is<JsonObjectConst>()) {
        ...
        p_s.autoOff.timer.enabled =
            ao["timer"]["enabled"] | false;
        p_s.autoOff.timer.minutes =
            ao["timer"]["minutes"] | 0;
        p_s.autoOff.offTime.enabled =
            ao["offTime"]["enabled"] | false;
        strlcpy(p_s.autoOff.offTime.time,
                ao["offTime"]["time"] | "",
                sizeof(p_s.autoOff.offTime.time));
        p_s.autoOff.offTemp.enabled =
            ao["offTemp"]["enabled"] | false;
        p_s.autoOff.offTemp.temp =
            ao["offTemp"]["temp"] | 0.0f;
    }

    // motion
    p_s.motion.pir.enabled =
        p_js["motion"]["pir"]["enabled"] | false;
    p_s.motion.pir.hold_sec =
        p_js["motion"]["pir"]["hold_sec"] | 0;
    p_s.motion.ble.enabled =
        p_js["motion"]["ble"]["enabled"] | false;
    p_s.motion.ble.rssi_threshold =
        p_js["motion"]["ble"]["rssi_threshold"] | -70;
    p_s.motion.ble.hold_sec =
        p_js["motion"]["ble"]["hold_sec"] | 0;
}
...
static void C10_fromJson_UserProfile(
    ...
    if (p_jp["autoOff"].is<JsonObjectConst>()) {
        JsonObjectConst ao =
            p_jp["autoOff"];
        p_up.autoOff.timer.enabled =
            ao["timer"]["enabled"] | false;
        p_up.autoOff.timer.minutes =
            ao["timer"]["minutes"] | 0;
        p_up.autoOff.offTime.enabled =
            ao["offTime"]["enabled"] | false;
        strlcpy(p_up.autoOff.offTime.time,
                ao["offTime"]["time"] | "",
                sizeof(p_up.autoOff.offTime.time));
        p_up.autoOff.offTemp.enabled =
            ao["offTemp"]["enabled"] | false;
        p_up.autoOff.offTemp.temp =
            ao["offTemp"]["temp"] | 0.0f;
    }

    // motion
    p_up.motion.pir.enabled =
        p_jp["motion"]["pir"]["enabled"] | false;
    p_up.motion.pir.hold_sec =
        p_jp["motion"]["pir"]["hold_sec"] | 0;
    p_up.motion.ble.enabled =
        p_jp["motion"]["ble"]["enabled"] | false;
    p_up.motion.ble.rssi_threshold =
        p_jp["motion"]["ble"]["rssi_threshold"] | -70;
    p_up.motion.ble.hold_sec =
        p_jp["motion"]["ble"]["hold_sec"] | 0;
}
...
bool CL_C10_ConfigManager::patchSchedulesFromJson(
    ...
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] Schedules patched (PUT). Dirty=true");
...
bool CL_C10_ConfigManager::patchUserProfilesFromJson(
    ...
    JsonArrayConst arr =
        p_patch["userProfiles"]["profiles"].as<JsonArrayConst>();
    if (arr.isNull()) {
        C10_MUTEX_RELEASE();
        return false;
    }
    ...
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] UserProfiles patched (PUT). Dirty=true");
...
bool CL_C10_ConfigManager::patchWindProfileDictFromJson(
    ...
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] WindProfileDict patched (PUT). Dirty=true");
...
int CL_C10_ConfigManager::addScheduleFromJson(const JsonDocument& p_doc) {
    ...
    JsonObjectConst js =
        p_doc["schedule"].is<JsonObjectConst>()
    ...
    _dirty_schedules = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] Schedule added (index=%d)", v_index);

    C10_MUTEX_RELEASE();
    return v_index;
}

bool CL_C10_ConfigManager::updateScheduleFromJson(uint16_t p_id,
                                                  const JsonDocument& p_patch) {
    C10_MUTEX_ACQUIRE_BOOL();
    ...
    JsonObjectConst js =
        p_patch["schedule"].is<JsonObjectConst>()
            ? p_patch["schedule"].as<JsonObjectConst>()
    ...
    _dirty_schedules = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] Schedule updated (id=%d, index=%d)",
                       p_id, v_idx);
...
bool CL_C10_ConfigManager::deleteSchedule(uint16_t p_id) {
    ...
    _dirty_schedules = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] Schedule deleted (id=%d, index=%d)",
                       p_id, v_idx);
...
int CL_C10_ConfigManager::addUserProfilesFromJson(const JsonDocument& p_doc) {
    ...
    JsonObjectConst jp =
        p_doc["profile"].is<JsonObjectConst>()
...
bool CL_C10_ConfigManager::updateUserProfilesFromJson(
    uint16_t            p_id,
    const JsonDocument& p_patch) {
    ...
    JsonObjectConst jp =
        p_patch["profile"].is<JsonObjectConst>()
            ? p_patch["profile"].as<JsonObjectConst>()
            : p_patch.as<JsonObjectConst>();
    ...
    _dirty_userProfiles = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] UserProfile updated (id=%d, index=%d)",
                       p_id, v_idx);
...
bool CL_C10_ConfigManager::deleteUserProfiles(uint16_t p_id) {
    ...
    _dirty_userProfiles = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] UserProfile deleted (id=%d, index=%d)",
                       p_id, v_idx);
...
int CL_C10_ConfigManager::addWindProfileFromJson(const JsonDocument& p_doc) {
    ...
    JsonObjectConst v_js =
        p_doc["preset"].is<JsonObjectConst>()
...
bool CL_C10_ConfigManager::updateWindProfileFromJson(
    uint16_t            p_id,
    const JsonDocument& p_patch) {
    ...
    JsonObjectConst v_js =
        p_patch["preset"].is<JsonObjectConst>()
            ? p_patch["preset"].as<JsonObjectConst>()
            : p_patch.as<JsonObjectConst>();
    ...
    _dirty_windProfile = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] WindPreset updated (index=%d)",
                       p_id);
...
bool CL_C10_ConfigManager::deleteWindProfile(uint16_t p_id) {
    ...
    _dirty_windProfile = true;
    CL_D10_Logger::log(EN_L10_LOG_INFO,
                       "[C10] WindPreset deleted (index=%d)",
                       p_id);
