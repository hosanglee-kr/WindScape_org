...
void SC10_init() {
	...
	// 기존: g_SC10_control.begin();
	
	// ✅ 변경: g_SC10_control.sim.begin(g_P10_pwm); 로직 제거 (CT10::begin()에서 이미 호출함)
	// g_SC10_control.sim.begin(g_P10_pwm); 
	
	// CT10 내부에서 시뮬레이션 active 설정 (초기화)
...
void SC10_run() {
	...
	// ✅ 변경: Simulation 2Hz 로직 제거 (CT10::tick()에서 40ms 주기로 처리)
	/*
	...
	*/
	...
	// ✅ 변경: ControlManager 루프 (정적 인터페이스 사용)
	//g_SC10_control.tick();
